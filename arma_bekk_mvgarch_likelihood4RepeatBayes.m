function [LLF,likelihoods, Ht, errors]= arma_bekk_mvgarch_likelihood4RepeatBayes(parameters, data, p, q, Nr, Nl, priors, sig)
...
% end
LLF = 0.5*(LLF);
% with Bayesian priors (priors, sig)\
sig = sig .* eye(size(parameters,1));
LLF = LLF + 0.5 * log(det(sig)) + 0.5 * (parameters - priors)' * sig^(-1) *(parameters - priors);
% Bayes = 0.5 * log(det(sig)) + 0.5 * (parameters - priors)' * sig^(-1) *(parameters - priors)
